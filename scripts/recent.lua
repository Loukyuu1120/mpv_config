...
function read_log(func)
    ...
    for line in f:lines() do
        if not line:match("^%s*$") then
            local success, result = pcall(func, line)
...
function write_log(delete)
    ...
    local temp_path = o.log_path .. ".tmp"
    local f = io.open(temp_path, "w+")
    if not f then 
        print("Error: Cannot open temp file for writing")
        return 
    end
    ...
    if not delete then
        local date_str = os.date(o.date_format)
        f:write("[" .. date_str .. "] \"" .. cur_title .. "\" | " .. cur_path .. "\n")
    end
    ...
    local success, err = os.rename(temp_path, o.log_path)
    if not success then
        print("Error renaming history log: " .. (err or "unknown"))
    end
